...
const SolmarkPlacePreviewPage = () => {
    ...
    const{list, setList, places, setPlaces} = useSolmarkStore();
  ...
  return (
    <div>
      <TitleHeader title={list.collectionName||'저장 리스트'} center onClick={handleClick} />
      <div className='pt-58'>
        <p className='text-primary-950 text-sm py-24 px-16 pb-8'>장소 {places.length}개</p>
      </div>
    </div>
  );
}
